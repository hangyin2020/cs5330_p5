"""
    ...
    task 1 abcd Build and train a network to recognize digits
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % log_interval == 0:
            ...
            torch.save(network.state_dict(),
                       '/Users/hangyin/results/model.pth')
            torch.save(optimizer.state_dict(),
                       '/Users/hangyin/results/optimizer.pth')
...
def show_prediction(network, example_data, data_size, row, col):
    with torch.no_grad():
...
def main(argv):
    ...
    print(f"example_data shape is: {example_data.shape}")
    ...
    show_prediction(network, example_data, 6, 3, 2)
